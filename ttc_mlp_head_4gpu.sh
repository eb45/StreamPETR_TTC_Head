#!/bin/bash
#SBATCH --job-name=streampetr_ttc_p3_4gpu
#SBATCH --output=logs/%x_%j.out
#SBATCH --error=logs/%x_%j.err
# 4× GPU Phase 3: frozen StreamPETR + TTC MLP (nuScenes mini by default).
# Uses projects/configs/StreamPETR/stream_petr_vov_ttc_frozen_20e_4gpu.py
...
# Same env vars as ttc_mlp_head.sh (AUTO_PREP_DATA, NUSCENES_*, STREAMPETR_*, WORK_DIR, USE_WANDB, …).
...
WORK_DIR="${WORK_DIR:-./work_dirs/streampetr_ttc_frozen_20e_tiered_4gpu}"
...
CONFIG_4GPU="projects/configs/StreamPETR/stream_petr_vov_ttc_frozen_20e_4gpu.py"
NUM_GPUS=4

echo "[train] 4-GPU dist  config=${CONFIG_4GPU}  work-dir=${WORK_DIR}  (global batch ≈ ${NUM_GPUS}× batch_size)"
bash tools/dist_train.sh "${CONFIG_4GPU}" "${NUM_GPUS}" \
  --work-dir "${WORK_DIR}" \
  "$@"
