#!/bin/bash
# TTC MLP eval: (1) tools/eval_ttc_mlp.py mean loss + bar chart
#                (2) tools/eval_ttc_breakdown.py per-class / per–GT-bin MAE (JSON + PNG)
#                (3) optional: tools/compare_ttc_scene.py — one scene, GT vs physics-from-bbox vs MLP
#
# Slurm (from repo root):
#   sbatch run_eval_ttc_iter56260.sh
#   CHECKPOINT=work_dirs/.../iter_XXX.pth MAX_BATCHES=100 sbatch run_eval_ttc_iter56260.sh
#
...
#   bash run_eval_ttc_iter56260.sh
#
# Optional env: CONFIG, CHECKPOINT (or CKPT), ANN_FILE, MAX_BATCHES, GPU_ID,
#   BREAKDOWN_MAX_BATCHES (default 50; breakdown can be slower than loss-only eval),
#   SKIP_TTC_BREAKDOWN=1 to skip eval_ttc_breakdown.py (still runs compare_ttc_scene if SCENE_TOKEN is set),
#   SCENE_TOKEN (default: hex below), PRETRAINED_BASELINE, COMPARE_SAVE_DIR, SKIP_COMPARE_SCENE,
#   NUSCENES_ROOT or COMPARE_DATA_ROOT — passed as --data-root to eval_ttc_mlp, eval_ttc_breakdown,
#     and compare_ttc_scene (dataset + CAM_FRONT PNGs). Default: unset → cfg ./data/nuscenes/
...
#   sbatch --nodelist=hostname run_eval_ttc_iter56260.sh
...
# Same nuScenes root for eval dataloading + compare camera PNGs (override if data is not under ./data/nuscenes/)
NUSCENES_ROOT_RESOLVED="${COMPARE_DATA_ROOT:-${NUSCENES_ROOT:-}}"
# Default scene for tools/compare_ttc_scene.py — must exist in ANN_FILE (val split here has 2 scenes).
# Alternate val scene: 325cef682f064c55a255f2625c533b75  (override: SCENE_TOKEN=... bash run_eval_ttc_iter56260.sh)
SCENE_TOKEN="${SCENE_TOKEN:-fcbccedd61424f1b85dcbf8f897f9754}"

[[ -f "$(pwd)/data/nuscenes/ttc_gt_labels_v1_0_mini.pkl" ]] && export STREAMPETR_TTC_PKL="$(pwd)/data/nuscenes/ttc_gt_labels_v1_0_mini.pkl"
...
# Optional: one-scene table (GT vs physics TTC from predicted bbox vs TTC MLP)
if [[ -n "${SCENE_TOKEN:-}" ]] && [[ "${SKIP_COMPARE_SCENE:-0}" != "1" ]]; then
  echo ""
  echo "=== Per-scene TTC compare (tools/compare_ttc_scene.py) ==="
